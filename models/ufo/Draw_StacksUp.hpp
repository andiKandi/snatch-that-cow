 ...
 */
#ifndef DRAW_STACKSUP_HPP
#define DRAW_STACKSUP_HPP

float yRotationAngleStacksUpHullUp = 0.0f; // The angle of rotation for our object

Model* stacksUp = new Model();
...
void Draw_StacksUp(){
    ...
    for (unsigned int i = 0; i < stacksUp->getMeshes().size(); i++) {
        stacksUp->Draw(stacksUp->getMeshes().at(i));
    }
    glPopMatrix();
}

#endif //DRAW_STACKSUP_HPP
